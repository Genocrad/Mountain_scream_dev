...
function DungeonWallSpawner:SpawnArenaTeleporter()
  ...
  while success ~= true do
    ...
    print("TRYING TO GENERATE PEAK",x,z,TheWorld.Map:GetTileAtPoint(x, 0, z), self.exits[8]:GetDistanceSqToPoint(x,0,z)) 
    if IsMsTile(TheWorld.Map:GetTileAtPoint(x, 0, z)) and self.exits[8]:GetDistanceSqToPoint(x,0,z) > 128 then
  ...
  for i = -7, 7 do 
    for j = -arena_width[i+8], arena_width[i+8] do
      TheWorld.Map:SetTile(tile_x + i + TUNING.MS_TERRAFORMER_OFFSET_X[8] + TUNING.MS_TERRAFORMER_OFFSET_X[9], tile_z + j + TUNING.MS_TERRAFORMER_OFFSET_Y[8] + TUNING.MS_TERRAFORMER_OFFSET_Y[9], WORLD_TILES.MS_BRICK) 
    end
  end
  local light = SpawnPrefab("light_fake_overworld")
  ...
  while success ~= true do
    ...
    if TileGroupManager:IsLandTile(TheWorld.Map:GetTileAtPoint(x, 0, z)) and not IsMsTechnicalTile(TheWorld.Map:GetTileAtPoint(x, 0, z)) then
